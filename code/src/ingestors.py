    ...
    def ingest_pages(self, filename_pattern: str, all: bool = True, max_pages: int = None, delay_seconds: int = 0) -> None:
        ...
        while all or (max_pages is not None and page <= max_pages):
            try:
                ...
                self.s3.upload_fileobj(file_obj, 'floriparentpricing', file_path) # warning: esta variável deve ser substituída para receber da classe ou da invocação da função
